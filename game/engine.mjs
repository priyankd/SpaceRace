...
export const OBSTACLES = [
 ...
 {z:4400,x:0,type:'asteroid'}, {z:5700,x:0,type:'laser',phase:0},
 {z:6800,x:.45,type:'rock'}, {z:7600,x:0,type:'asteroid'},
 {z:8600,x:0,type:'laser',phase:2}, {z:10000,x:-.42,type:'rock'},
 ...
 {z:15700,x:0,type:'laser',phase:1,onFor:1.8},
 ...
 {z:12200,x:0,y:240,type:'laser',phase:1,onFor:1.8},
 {z:15200,x:0,y:360,type:'rock'},
 {z:18200,x:0,y:260,type:'asteroid',rate:1.8},
 {z:21000,x:0,y:250,type:'laser',phase:3,onFor:2},
...
export function bestKey(id='moon'){return `space-race-best-flyby-v1-${getLevel(id).id}`;}
export function createState(levelId='moon'){return {levelId:getLevel(levelId).id,mode:'briefing',z:0,x:0,speed:0,time:0,boost:100,y:8,vy:0,recoveries:0,invincible:0,found:/** @type {number[]} */ ([]),notice:'',noticeTime:0,boosting:false,jumps:0};}
export function laserOn(o,time){return (time+(o.phase||0))%6<(o.onFor??1.6);}
...
export function recover(s){s.x*=.6;s.speed=Math.min(s.speed,100);s.y=Math.max(flightBounds(s.z,s.levelId).minY,Math.min(flightBounds(s.z,s.levelId).maxY,s.y));s.vy=0;s.invincible=1.2;s.recoveries++;announce(s,'Shield absorbing impact');}
export function step(s,input,dt){
 if(s.mode!=='racing')return;
 const level=getLevel(s.levelId);
 dt=Math.min(.05,Math.max(0,dt));s.time+=dt;s.noticeTime=Math.max(0,s.noticeTime-dt);s.invincible=Math.max(0,s.invincible-dt);
 const previous=s.z;
 ...
 for(const o of level.obstacles){if(previous<o.z&&s.z>=o.z&&s.invincible===0){
 const relativeY=s.y-(o.y??0),size=asteroidSize(o);
 const hit=o.type==='laser'?laserOn(o,s.time)&&Math.abs(s.x)<1.12&&relativeY>-105&&relativeY<310:Math.abs(s.x-obstacleX(o,s.time))<(size.width/2+35)/600&&relativeY>-105&&relativeY<size.height+10;
 if(hit){recover(s);return;}
 }}
